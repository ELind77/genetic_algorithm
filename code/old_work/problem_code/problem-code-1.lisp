...
;;; Name: Eric Lind
;;; ========================================
;;; FILE:  problem-code-1.lisp
;;; ========================================
;;; This is an early version of the problem-specific code
;;; for the genetic-programming system.
...
;;;  RANDOM-FROM-RANGE
;;;============================================================
;;; Generates a random number in the specified range (inclusive).
;;;   INPUTS:   START, int
;;;             END, int
;;;   OUTPUT:   int
...
system."
  ...
  (labels 
	 ...
	 (cond 
	  ...
	  ((null listy)
	   (reverse acc))
	  ;; Recursive case, add to accumulator
	  (t
	   (flatten (rest listy) (cons (first listy) acc))))))
    ...
    (run-shell-command "~/cs365/final_project/code/kernel/trial-maker2.sh `cat trial_parameters.txt`")
	...
	(dotimes (i num-fitness-cases)
	  ;; Create a flattened list from each line
	  (let ((current-line (flatten (read stream) nil)))
	    (format t "~%~A~%" current-line)
	    
	    ;; Now walk through LINE
	    (dotimes (j 12)
...
(defstruct fitness-case
  ...
  3-day-slope				;The slope of the closing over the last 3 days
...
;;;  WRAPPER
;;;============================================================
;;; Doesn't really do anything, but the kernel asked for it
;;; (i.e. Koza asked for it).
;;; It can be used to transform the result from runnign the 
;;; program into some other desired format.

(defun wrapper (result-from-program)
  result-from-program)
...
is the sum of the entries in the vector [sum of the errors]."
  ...
  (let ((raw-fitness-vector (make-array (length fitness-cases)))
	...
	(setf (svref standard-fitness-vector index) standardized-fitness)
	...
	(when (<= percent-diff 5) (incf hits))))
	
    ;; Return the fitnesses and hits
    (let ((std-fitness-avg (float 
			    (/ sum-of-standardized-fitnesses 
			       (length fitness-cases))))
	  (std-fitness-percent-avg (float 
				    (/ sum-of-standardized-percent
				       (length fitness-cases)))))
...
(defun define-parameters-for-stock-prediction ()
  ...
  (setf *num-fitness-cases* 10)
  (setf *max-depth-for-new-indis* 6)
