...
;;; problem.  The basis for these functions was the simple
;;; LISP code in John R. Koza's book.
...
;; Protected Functions
(load (compile-file "protected-functions-2"))
  ...
  (multiple-value-bind (function-set-creator
	...
	(if log?
	    (progn 
	      ...
	      (with-open-file (stream "gp-log.txt" :direction :output
			       :if-exists :supersede)
		(fresh-line stream))))
