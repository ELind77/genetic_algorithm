...
;;; MY GLOBALS...
;;;===================================================
;;; I added some of my own globals for testing and such
...
not ~S" size-of-pop)
  ...
  (multiple-value-bind (function-set-creator
	...
	;;(format t "~3%Best of run:  ~A~%" *best-of-run-individual*)
	;;(format t "Generation:      ~A~%" 
	;;	*generation-of-best-of-run-individual*)
	
	
	;; If desired, print population
	;;(if print-pop?
 ...
 (let ((new-male-depth (tree-depth (first new-male)))
  ...
  (values
   ...
   (if (or (= 1 new-male-depth)
           (> new-male-depth *max-depth-for-individuals-after-crossover*))
   ...
   (if (or (= 1 new-female-depth)
           (> new-female-depth *max-depth-for-individuals-after-crossover*))
        female
    ;; Else
    (first new-female))))) 
